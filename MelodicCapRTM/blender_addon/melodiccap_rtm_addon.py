...
def set_bone_world_position(rig, bone, world_pos):
    ...
    For root-level Rigify IK targets (parented to root at origin), parent
    space equals armature space, so this is correct.
    """
...
def measure_rig_proportions(rig):
    ...
    # Hip center rest position (for offset calculation)
...
def compute_spine_fk_chain(rig, spine_points, armature_inv_33):
    ...
    # Cumulative rotation: starts as the first bone's parent rest orientation.
    # For spine_fk, its Rigify parent (hips/MCH bone) is not being FK-rotated
    # by us, so its rest orientation is the starting point.
...
class MELODICCAP_OT_import_json(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        log_every = max(1, len(frames) // 10)  # Log ~10 progress updates
        ...
        for frame_idx, frame_data in enumerate(frames):
            ...
            if torso and hip_center:
                # Scale hip position relative to ground (Z=0) using global scale
                scaled_hip = hip_center.copy()
                scaled_hip.z *= global_scale
                # XY motion also scaled so steps match rig proportions
