...
class TriangulationEngine:
    ...
    def _init_filter(self, idx):
        # Using simple 1D Kalman filters for each axis of each landmark
    ...
    def triangulate_pose(self, landmarks_a, landmarks_b, smooth=True):
        """
        landarks_a, landmarks_b: MediaPipe landmark objects
        returns: Dictionary of landmark_idx -> [X, Y, Z] (Blender Space)
        """
        ...
        for i in range(33):
            ...
            pt2 = np.array([[lm_b.x * 1280, lm_b.y * 720]], dtype=np.float32)
            
            # CRITICAL: Undistort BEFORE triangulation
    ...
    def triangulate_hands(self, hands_a, hands_b):
        ...
        for h_a, h_b in zip(hands_a, hands_b):
            pts_a, pts_b = [], []
            for i in range(21):
                pts_a.append([h_a.landmark[i].x * 1280, h_a.landmark[i].y * 720])
                pts_b.append([h_b.landmark[i].x * 1280, h_b.landmark[i].y * 720])
    ...
    def set_floor_plane(self, floor_normal, floor_point):
        ...
        print(f"Floor calibration set: Z-Offset = {self.floor_offset_z}")

    def apply_floor_offset(self, point_3d):
        """Translates a point to align with calibrated floor"""
        if hasattr(self, 'floor_offset_z'):
